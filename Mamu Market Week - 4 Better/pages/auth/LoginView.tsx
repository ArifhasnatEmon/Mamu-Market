...
// Handles both User and Merchant authentication (Login and Signup).
const LoginView: React.FC<{ initialVendorMode?: boolean }> = ({ initialVendorMode = false }) => {
  ...
  // Toggling between Customer and Vendor views
      ...
      <div className="flex-1 flex flex-col bg-gray-50 lg:bg-white lg:h-screen lg:overflow-y-auto">
        ...
        <form className="space-y-4" onSubmit={(e) => {
          if (authMode === 'signup') {
            ...
            // Phone validation (BD: 01XXXXXXXXX, 11 digits)
              ...
              <>
                ...
                <button
                  onClick={() => {
                    ...
                    // Generate a 6-digit PIN and store it (demo — shows in alert since no real email)
                    const pin = String(Math.floor(100000 + Math.random() * 900000));
                    setForgotPin(pin);
                    localStorage.setItem(`reset_pin_${forgotEmail.toLowerCase()}`, JSON.stringify({ pin, expiresAt: Date.now() + 10 * 60 * 1000 }));
                    // In a real app this would send an email — for demo we show it in the verify step
              ...
              <>
                ...
                <button
                  onClick={async () => {
                    ...
                    setToast('Password reset successful! Please log in.');
